...
class PostController extends Controller
{
    ...
    public function danh_muc_bai_viet(Request $request,$post_slug){
        ...
        foreach($catepost as $key => $cate){
            ...
            $meta_tittle = $cate->cate_post_name;
        ...
        ->with('slider',$slider)->with('category_post',$category_post)->with('post_cate',$post_cate);
    }
    ...
    public function bai_viet(Request $request,$post_slug){
        ...
        $post_cate = Post::with('cate_post')->where('post_status',0)->where('post_slug',$post_slug)->take(1)->get();

        foreach($post_cate as $key => $p){
            //seo 
            $meta_desc = $p->post_meta_desc; 
            $meta_keywords = $p->post_meta_keywords;
            $meta_tittle = $p->post_title;
